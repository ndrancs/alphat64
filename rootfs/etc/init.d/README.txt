Fatdog system services
======================

As Fatdog was originally derived from Puppy Linux, Fatdog inherits most of 
Puppy's machinery for management of system services ("daemons"), 
which is a modified BSD-style init (services are in /etc/init.d instead of /etc/rc.d)

The start-up and shutdown of Fatdog services is identical with Puppy, 
as noted in Puppy documentation below, with minor differences:
- Fatdog adds "status" call to all service (used by service manager); and
  in most cases add "restart" too.
- From Fatdog 612 onwards, services are stopped in reverse-alphabetical order.

Note regarding udev: 
Fatdog does not use pup_event_frontend_d, instead, it uses udev directly. 
In Fatdog udev is an essential system services and should NEVER be killed. 
It is started directly by rc.sysinit.

For more information please read HACKING document in Fatdog fossil repository.

========= Original documentation from puppy linux =======
...
Service Management provided in the BootManager (see System menu).
